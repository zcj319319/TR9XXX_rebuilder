...
class VSI_INIT_CONFIG(Structure):
    _fields_ = [("ControlMode", c_ubyte),
                # 0-Hardware control (full-duplex mode),1-Hardware control (half duplex mode),2-Software control (half duplex mode),3-Single bus model, data input and output for MOSI
                ...
                ("ClockSpeed", c_int),
                # Hardware(36000000,18000000,9000000,4500000,2250000,1125000),Software(1000 To 1000000)
                ];
...
def VSI_WriteReadBytes(DevType, DevIndex, SPIIndex, pWriteData, WriteLen, pReadData, ReadLen):
    try:
        ...
        print('%s: %s' % (args.__class__.__name__, args))
        exit()
    return GinkgoLib.VSI_WriteReadBytes(c_int(DevType), c_int(DevIndex), c_int(SPIIndex), pWriteData, c_short(WriteLen),
                                        pReadData, c_short(ReadLen))


# Binary mode write data to SPI
def c(DevType, DevIndex, SPIIndex, pWriteBitStr):
    try:
        assert DevType == VSI_USBSPI, 'Device type error!'
        assert 0 <= SPIIndex <= 4, 'SPI index error!'
    except AssertionError as args:
...
def VSI_WriteReadBits(DevType, DevIndex, SPIIndex, pWriteBitStr, pReadBitStr, ReadBitsNum):
    try:
        ...
        print('%s: %s' % (args.__class__.__name__, args))
        exit()
    return GinkgoLib.VSI_WriteReadBits(c_int(DevType), c_int(DevIndex), c_int(SPIIndex), pWriteBitStr, pReadBitStr,
                                       c_int(ReadBitsNum))
...
def VSI_BlockWriteBytes(DevType, DevIndex, SPIIndex, pWriteData, BlockSize, BlockNum, IntervalTime):
    try:
        ...
        print('%s: %s' % (args.__class__.__name__, args))
        exit()
    return GinkgoLib.VSI_BlockWriteBytes(c_int(DevType), c_int(DevIndex), c_int(SPIIndex), pWriteData,
                                         c_ushort(BlockSize), c_ushort(BlockNum), c_uint(IntervalTime))
...
def VSI_BlockReadBytes(DevType, DevIndex, SPIIndex, pReadData, BlockSize, BlockNum, IntervalTime):
    try:
        ...
        print('%s: %s' % (args.__class__.__name__, args))
        exit()
    return GinkgoLib.VSI_BlockReadBytes(c_int(DevType), c_int(DevIndex), c_int(SPIIndex), pReadData,
                                        c_ushort(BlockSize), c_ushort(BlockNum), c_uint(IntervalTime))


# Block mode write&read
def VSI_BlockWriteReadBytes(DevType, DevIndex, SPIIndex, pWriteData, WriteBlockSize, pReadData, ReadBlockSize, BlockNum,
                            IntervalTime):
    try:
        assert DevType == VSI_USBSPI, 'Device type error!'
        assert 0 <= SPIIndex <= 4, 'SPI index error!'
        ...
        print('%s: %s' % (args.__class__.__name__, args))
        exit()
    return GinkgoLib.VSI_BlockWriteReadBytes(c_int(DevType), c_int(DevIndex), c_int(SPIIndex), pWriteData,
                                             c_ushort(WriteBlockSize), pReadData, c_ushort(ReadBlockSize),
                                             c_ushort(BlockNum), c_uint(IntervalTime))
